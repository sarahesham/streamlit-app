...
async def _handle_accordion_pagination(
    ...
    """
    Handle accordion-style pagination by expanding sections and extracting courses per section.
    
    Returns:
        List of course URLs extracted from all accordion sections
    """
    course_urls = []
    selector = config.get("pagination_selector")
    course_selector = config.get("course_selector")
    
    if not selector or not course_selector:
        return course_urls
    ...
    try:
        ...
        for idx, btn in enumerate(accordion_buttons):
            try:
                btn_text = await btn.inner_text() or f"section_{idx}"
                aria_expanded = await btn.get_attribute("aria-expanded")
                
                # Click only if not already expanded
                if aria_expanded != "true":
                    # Try to dismiss cookie dialog if it appears (before clicking)
                    try:
                        cookie_dialog = await page.query_selector("#CybotCookiebotDialog")
                        if cookie_dialog:
                            is_visible = await cookie_dialog.is_visible()
                            if is_visible:
                                ...
                                if accept_btn:
                                    ...
                                    print(f"  ✓ Dismissed cookie dialog before section {idx + 1}")
                    except:
                        pass
                    
                    # Use force click if normal click fails (for elements behind overlays)
                    try:
                        ...
                        try:
                            ...
                            print(f"  ⚠️ Could not click section {idx + 1}, skipping")
                            continue
                    
                    # Wait for courses to appear in this section
                    try:
                        await page.wait_for_selector(course_selector, timeout=5000)
                    except PWTimeoutError:
                        pass  # Continue anyway
                    await page.wait_for_timeout(800)  # Delay for content to render
                
                # Extract courses from current section
                section_courses = await page.query_selector_all(course_selector)
                section_urls = []
                
                for el in section_courses:
                    try:
                        href = await el.get_attribute("href")
                        if not href:
                            continue
                        
                        # Clean URL if needed (for Buckingham - remove tracking parameters)
                        if "buckingham.ac.uk" in base_url:
                            # Remove tracking parameters like _gl, _ga, _up, etc.
                            if "?" in href:
                                parsed = urlparse(href)
                                # Check if URL has tracking params
                                if any(param in parsed.query for param in ["_gl", "_ga", "_up", "gclid", "utm_"]):
                ...
                course_urls.extend(section_urls)
                print(f"  Section '{btn_text[:50]}...': Found {len(section_urls)} courses")
                
            except Exception as e:
                print(f"Error processing accordion section {idx}: {e}")
                continue
        
        # Deduplicate
        unique_urls = list(dict.fromkeys(course_urls))
        print(f"Total unique courses extracted: {len(unique_urls)}")
        return unique_urls
    except Exception as e:
        print(f"Error in accordion pagination: {e}")
        return course_urls
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if pagination_type == "single_page":
                ...
                # Accordion - expand sections and extract courses per section
                await page.goto(university_url, wait_until="domcontentloaded")
                pagination_urls.append(university_url)
                
                # Handle cookie consent dialog (for Buckingham and similar sites)
                try:
                    ...
                    pass  # Continue if cookie dialog not found
                
                # Wait for accordion buttons to appear
                wait_selector = config.get("wait_selector", config.get("pagination_selector"))
                if wait_selector:
                    try:
                        await page.wait_for_selector(wait_selector, timeout=30000)
                    except PWTimeoutError:
                        print("Warning: Accordion buttons not found, proceeding anyway")
                
                # Handle accordion and extract courses per section
                course_urls = await _handle_accordion_pagination(page, config, base_url)
                all_urls.extend(course_urls)
